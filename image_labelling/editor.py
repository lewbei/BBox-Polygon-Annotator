...
from image_labelling.exporter import convert_to_coco_format, convert_to_pascal_voc_format, convert_to_csv_format
...
class BoundingBoxEditor:
    ...
    # --------------------------------------------------
    def _init_project_attributes(self):
        self.model = None
        self.cancel_event = None
        self.folder_path = self.project["dataset_path"]
    ...
    def _restore_last_opened_image(self):
        if "last_opened_image_relative" in self.project:
            last_image_rel = self.project["last_opened_image_relative"]
            if last_image_rel:
                last_full = os.path.join(self.folder_path, last_image_rel)
                if os.path.exists(last_full) and last_image_rel in self.image_files:
                    try:
                        self.image_tree.selection_set(last_image_rel)
                        self.image_tree.focus(last_image_rel)
                        self.image_tree.see(last_image_rel)
                        self.load_image(last_full)
                    except tk.TclError:
                        logging.warning("Could not auto-select last opened image '%s' in tree.", last_image_rel)
                        if self.image_files:
    ...
    def _save_project_config(self):
        ...
        if not hasattr(self, 'project') or 'project_name' not in self.project:
            logging.error("Project name not found, cannot save project config.")
            return
        ...
        try:
            ...
            logging.error("Error saving project configuration to %s: %s", project_file_path, e, exc_info=True)
            ...
            if self.original_image is None:
                ...
                if disp_w > 0 and disp_h > 0: # Ensure dimensions are positive
                    try:
                        ...
                        logging.error("OpenCV error during resize in on_canvas_resize: %s", e, exc_info=True)
            ...
            if format == "coco":
                ...
                for image_path in annotated_images:
                    ...
                    if os.path.exists(full_image_path):
                        ...
                        if img is not None:
                            height, width = img.shape[:2]
                            xml_str = convert_to_pascal_voc_format(
